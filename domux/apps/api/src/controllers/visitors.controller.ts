...
export async function listVisitorsHandler(
  ...
  const visitors = await visitorsService.listVisitors(
    request.authUser!,
    filters,
  );
...
export async function authorizeVisitorHandler(
  request: FastifyRequest,
  reply: FastifyReply,
) {
  const input = parseOrThrow(authorizeVisitorSchema, request.body);
  const visitor = await visitorsService.authorizeVisitor(
    request.authUser!,
    input,
  );
...
export async function registerVisitorManuallyHandler(
  request: FastifyRequest,
  reply: FastifyReply,
) {
  const input = parseOrThrow(registerVisitorManuallySchema, request.body);
  const visitor = await visitorsService.registerVisitorManually(
    request.authUser!,
    input,
  );
  return reply.code(201).send(visitor);
...
export async function cancelVisitorHandler(
  request: FastifyRequest,
  reply: FastifyReply,
) {
...
export async function registerEntryHandler(
  request: FastifyRequest,
  reply: FastifyReply,
) {
  const { id } = request.params as { id: string };
...
export async function registerExitHandler(
  request: FastifyRequest,
  reply: FastifyReply,
) {
  const { id } = request.params as { id: string };
