...
import { packageFiltersSchema, registerPackageSchema } from '../schemas/packages.schema';
...
export async function listPackagesHandler(request: FastifyRequest, reply: FastifyReply) {
  ...
  const packages = await packagesService.listPackages(request.authUser!, filters);
  return reply.send(packages);
}

export async function registerPackageHandler(request: FastifyRequest, reply: FastifyReply) {
