...
export async function authorizeVisitor(actor: AuthUser, input: AuthorizeVisitorInput) {
...
export async function cancelVisitor(actor: AuthUser, visitorId: string) {
  const visitor = await prisma.visitor.findFirst({ where: { id: visitorId, tenantId: actor.tenantId as string } });
  if (!visitor) throw new NotFoundError('Visitante no encontrado');
  if (visitor.authorizedById !== actor.sub) throw new ForbiddenError('Solo puedes cancelar tus propias autorizaciones');
  if (visitor.status !== 'AUTHORIZED') throw new ConflictError('Solo se pueden cancelar visitantes en estado AUTHORIZED');

  const updated = await prisma.visitor.update({ where: { id: visitorId }, data: { status: 'CANCELLED' } });
  await recordAudit({
    userId: actor.sub,
    tenantId: actor.tenantId,
...
export async function registerVisitorManually(actor: AuthUser, input: RegisterVisitorManuallyInput) {
...
export async function registerEntry(actor: AuthUser, visitorId: string) {
  const visitor = await prisma.visitor.findFirst({ where: { id: visitorId, tenantId: actor.tenantId as string } });
  if (!visitor) throw new NotFoundError('Visitante no encontrado');
  if (visitor.status !== 'AUTHORIZED') throw new ConflictError('El visitante no está en estado AUTHORIZED');
...
export async function registerExit(actor: AuthUser, visitorId: string) {
  const visitor = await prisma.visitor.findFirst({ where: { id: visitorId, tenantId: actor.tenantId as string } });
  if (!visitor) throw new NotFoundError('Visitante no encontrado');
  if (visitor.status !== 'ENTERED') throw new ConflictError('El visitante no está en estado ENTERED');
...
/** Listado con reglas distintas por rol: ADMIN ve todo el historial, GATEKEEPER ve autorizados/entrados, RESIDENT ve solo los suyos. */
export async function listVisitors(actor: AuthUser, filters: VisitorFilters) {
  const tenantId = actor.tenantId as string;
  const unitFilter = filters.unitCode ? { unit: { code: filters.unitCode } } : {};
  ...
  if (actor.role === 'GATEKEEPER') {
    return prisma.visitor.findMany({
      where: { tenantId, status: filters.status ?? { in: ['AUTHORIZED', 'ENTERED'] }, ...unitFilter },
      include: { unit: true },
      orderBy: { createdAt: 'desc' }
    });
  }

  // RESIDENT: solo los visitantes que él mismo autorizó.
  return prisma.visitor.findMany({
    where: { tenantId, authorizedById: actor.sub, status: filters.status },
    orderBy: { createdAt: 'desc' }
  });
}
